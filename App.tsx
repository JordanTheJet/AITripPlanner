...
function AppComponent() {
  ...
  // Effect: Sync map markers with trip itinerary items from database
  const { currentTrip, items } = useTrip();
  useEffect(() => {
    // Only sync if we have a current trip and items
    ...
    // Update map store with trip markers
    if (tripMarkers.length > 0) {
      useMapStore.getState().setMarkers(tripMarkers);
    }
  }, [currentTrip, items]);
