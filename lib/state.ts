...
export const useLogStore = create<{
  ...
  saveChatMessage: (message: string, type: 'user' | 'ai') => Promise<void>;
}>((set, get) => ({
  turns: [],
  ...
  addTurn: (turn: Omit<ConversationTurn, 'timestamp'>) => {
    ...
    if (newTurn.text && newTurn.text.trim()) {
      const messageType = newTurn.role === 'user' ? 'user' : 'ai';
      get().saveChatMessage(newTurn.text, messageType).catch(console.error);
    }
  },
  ...
  saveChatMessage: async (message: string, type: 'user' | 'ai') => {
    try {
      // Dynamic import to avoid circular dependencies
      const { supabase } = await import('./supabase');
      const { useLobby } = await import('./stores/lobby-store');
      ...
      if (!lobby) return; // No lobby, skip saving
