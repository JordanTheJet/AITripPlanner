...
export const toolRegistry: Record<string, ToolImplementation> = {
  ...
  frameLocations,
  // Trip planning tools
  createTrip: async (args, context) => {
    const { tripToolHandlers } = await import('./trip-tool-handlers');
    return tripToolHandlers.createTrip(args, context);
  },
  addItineraryDay: async (args, context) => {
    const { tripToolHandlers } = await import('./trip-tool-handlers');
    return tripToolHandlers.addItineraryDay(args, context);
  },
  addItineraryItem: async (args, context) => {
    const { tripToolHandlers } = await import('./trip-tool-handlers');
    return tripToolHandlers.addItineraryItem(args, context);
  },
  updateBudget: async (args, context) => {
    const { tripToolHandlers } = await import('./trip-tool-handlers');
  ...
  // Export tools
  exportToGoogleMaps: async (args, context) => {
    const { exportToolHandlers } = await import('./export-tool-handlers');
    return exportToolHandlers.exportToGoogleMaps(args, context);
  },
  exportToCalendar: async (args, context) => {
    const { exportToolHandlers } = await import('./export-tool-handlers');
    return exportToolHandlers.exportToCalendar(args, context);
  },
